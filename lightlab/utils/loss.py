...
from .tal import TaskAlignedAssigner, make_anchors, dist2bbox, bbox_iou, bbox2dist
...
class FocalLoss:
    ...
    def __call__(self, pred: torch.Tensor, batch, gamma=1.5, alpha=0.25):
        label = batch["mask"]
        ...
        # moudlatin_factor = (1.0 - p_t) ** gamma
        # loss *= moudlatin_factor
        # if alpha > 0:
...
class ClassificationLoss:
    def __call__(self, preds, batch):
        loss = F.cross_entropy(preds, batch["cls"], reduction="sum") / 64
        return loss, loss.detach()
...
class DetectionLoss:
    ...
    def __init__(self, model, cfg: Settings):  # model must be de-paralleled
